...
var TavilySearchProvider = class {
	...
	*/
	constructor(resolveOptions, hybridSearch, deepseekDelegate) {
	...
	async search(request, signal) {
		...
		const secondaryResult = hybrid === void 0 ? void 0 : await this.runSecondarySearch(hybrid.run, request, signal);
		const apiKey = await resolveRequestApiKey(options, signal);
		return mergeSearchResults(secondaryResult, await this.tavilySearch(request, signal, options, apiKey), hybrid?.secondaryFirst ?? true);
	}
	/**
...
*/
function mergeSearchResults(secondary, tavily, secondaryFirst = true) {
	if (secondary === void 0) return tavily;
	const seen = /* @__PURE__ */ new Set();
	const sources = (secondaryFirst ? [...secondary.sources, ...tavily.sources] : [...tavily.sources, ...secondary.sources]).filter((source) => {
		if (seen.has(source.url)) return false;
		seen.add(source.url);
		return true;
	});
	const content = (secondaryFirst ? [secondary.content, tavily.content] : [tavily.content, secondary.content]).filter((value) => typeof value === "string" && value.length > 0).join("\n\n");
	return {
...
const Config = z.object({
	...
	searchMode: z.union([
		...
		"deepseek-first",
		"tavily-first"
	]).description("Search composition: Tavily-only, DeepSeek-first, or Tavily-first with merge."),
...
*/
function hybridFor(ctx, current) {
	const mode = current().searchMode ?? "tavily-only";
	if (mode === "deepseek-first") return {
		run: hybridDeepSeekSearch(ctx),
		secondaryFirst: true
	};
	if (mode === "tavily-first") return {
		run: hybridDeepSeekSearch(ctx),
...
function apply(ctx, config) {
	...
	const provider = new TavilySearchProvider(() => resolveOptions(ctx, current(), entry), () => hybridFor(ctx, current), () => hybridDeepSeekSearch(ctx));
...
function warnIfNotActiveSearchProvider(ctx) {
	...
	if (active !== void 0 && active !== "tavily") logger.warn(`web_search is currently configured to the "${active}" search provider, NOT Tavily. SearchMode only controls how Tavily merges DeepSeek once selected — to actually use this plugin, ${FIX}`);
